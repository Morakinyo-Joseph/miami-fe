...
export function Camera({ onCapture }: CameraProps) {
  ...
  const [stream, setStream] = useState<MediaStream | null>(null);
  const [torch, setTorch] = useState(false);
  const [focusRange, setFocusRange] = useState<{ min: number; max: number; current: number }>({ min: 0, max: 10, current: 5 });
        ...
        if (videoRef.current) {
          ...
          if (capabilities.focusDistance) {
            ...
            setFocusRange({
              ...
              current: capabilities.focusDistance.step || 5
            });
          }
        }
      } catch (err) {
  ...
  const adjustFocus = async (value: number) => {
    if (!stream) return;

    const track = stream.getVideoTracks()[0];
    try {
      await track.applyConstraints({
        advanced: [{ focusDistance: value } as any]
      });
      setFocusRange(prev => ({ ...prev, current: value }));
    } catch (err) {
      console.error('Error adjusting focus:', err);
    }
  };
  ...
  return (
    <div className="relative w-full max-w-md mx-auto">
      ...
      {hasFocusControl && (
        <div className="absolute left-4 top-1/2 -translate-y-1/2 h-48 flex flex-col items-center">
          <input
            type="range"
            min={focusRange.min}
            max={focusRange.max}
            step={0.1}
            value={focusRange.current}
