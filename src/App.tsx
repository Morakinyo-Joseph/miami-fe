          ...
          <>
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
              <div className="relative">
                ...
                <button
                  onClick={() => setIsFingerSelectorOpen(!isFingerSelectorOpen)}
                    ...
                    {fingers.map((finger) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-50 transition-colors ${
                          selectedFinger === finger ? 'bg-blue-50 text-blue-600' : ''
                        }`}
                      >
                        <span className="text-xl">{fingerIcons[finger]}</span>
                        <span className="capitalize">{finger} Finger</span>
                      </button>
                    ))}
                  </div>
                )}
...
function App() {
  ...
  const handleRefresh = async () => {
    if (!clientId || !processedFingerprint) return;
    setMessage(null);
    try {
      const details = await getFingerprintDetails(clientId, processedFingerprint.finger_id);
      setProcessedFingerprint(details.data[0]);
    } catch (error) {
      setMessage({ type: 'error', text: 'Failed to refresh fingerprint details.' });
    }
  };
